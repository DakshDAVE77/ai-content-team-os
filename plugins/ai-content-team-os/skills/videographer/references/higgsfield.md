...
- Export only — **never publish from Higgsfield.** The clip is downloaded and posted by
  `post-runner`, so that one route carries every gate and every log write. Do not record
  a publishing capability as usable.

...
**Choosing the avatar route at the top of a run satisfies this**, as does asking for the
Reel in so many words. Either is him commissioning a render of a script this team wrote
to his brief, and the read then happens on the finished clip.

**A console POST click does not.** POST authorizes publication of content that already
exists, and it never commissions a paid render — see `publisher/references/console.md`
and CODEX-INTEGRATION.md. A piece that reaches the console with no clip is not
publishable and is reported, not rendered.

Record who asked, and when, in the piece file. If an approver identity is recorded in
`brand/avatar-motion.md`, check the request against it; if none is recorded, the
operator's own instruction stands and the identity is captured for the audit trail.
**Do not refuse on identity alone.**
...
So: **picking the avatar route at the top of a run, or asking for the Reel, is the
go-ahead. Render immediately. Do not pause to paste the script and wait for a second
yes.**
...
```
route chosen, or render asked for  →  render  →  clip into chat  →  stop
                                                                     ↓
                                  he says "post the video"  →  upload  →  permalink
```
...
- **Publish on an assumed yes.** A render is not a post. Nothing reaches a live account
  ...
  that word.
- **Stop an approved render to ask a question.** The avatar route chosen

...
built to run interactively — it needs him to name the topic, and the render needs an
open browser —
...
Posting is a separate step and a separate skill. He clicks POST in the local console;
that saves the exact selection and authorizes it. When he then says **"post"** in chat,
`post-runner` dispatches every selected, unpublished platform through the Browser
plugin. Do not attempt to post during this run, and never treat a click as evidence
that something went live — only a permalink is.

If the Browser plugin is unavailable, or the signed-in account does not match the
configured handle, the selections stay `awaiting_browser` and nothing is lost — the
console is unaffected and the run still ships.

**The POST button never commissions a Reel.** It authorizes publication of content that
already exists. What the Instagram options carry depends on Step 0:

- **A route ran and produced a file** — the video already exists and the console carries
  it. The POST click is a selection and nothing more, and `post-runner` uploads that
  exact file. **It does not render again.** A second render of a piece that already has
  one is billed twice and can come back subtly different from the clip that was approved.
- **The avatar route was chosen but the browser was unreachable, or a script tripped the
  refusal list** — there is no clip. The option ships with its script visible but is
  **not offered for posting**, and the receipt says so under NEEDS YOU. Getting the Reel
  made is a separate instruction from him, which runs through `avatar`.
- **No video route** — the Instagram options are caption plus cover. The button posts;
  nothing is commissioned.

Record which of the three applies on the saved request, so `post-runner` reads it

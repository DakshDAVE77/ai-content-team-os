...
| Presenter video — the operator on screen | a video connector (`~~video generation`, HeyGen) **plus** `brand/avatar-motion.md` with a confirmed `avatar_id` and `voice_id` | Reels ship as script + stills, or the operator films |
